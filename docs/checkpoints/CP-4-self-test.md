...
### 1. ⚠️ 打包失败:winCodeSign 解压报"无创建符号链接权限"

**现象**:`npm run build` 在解压 `winCodeSign-2.6.0` 工具时失败:
```
...
**根因**:electron-builder 把 macOS .dylib 也下载下来(用于跨平台签名工具链),但 Windows 上创建符号链接默认需要管理员权限。

**Vite 编译阶段成功**:`npx electron-vite build` 在 `out/main` / `out/preload` / `out/renderer` 下产物全部正常。

**解决方法(任选其一)**:
- **方法 A(推荐)**:启用 Windows 开发者模式 — 设置 → 隐私和安全性 → 开发者选项 → 打开"开发者模式"。开启后无需管理员权限就能创建符号链接,然后正常 `npm run build`
- **方法 B**:以管理员身份打开 PowerShell 跑 `npm run build`
- **方法 C**:删除 `%LOCALAPPDATA%\electron-builder\Cache\winCodeSign\` 整个目录后重试(解压一次成功后会 cache,后续不需要再解压)
...
### 3. uiZoom 通过 webFrame.setZoomFactor 实现
...
- **干净 Win11 安装运行** — 没有干净 VM 或独立机器
- **完整打包产物可用性** — 见上方已知问题 #1,需要开发者机器开启开发者模式后跑 `npm run build`
